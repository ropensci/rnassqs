...
For example, to request corn yields for all years since 2000, you would use something like:
    ...
    df = get_nass(params=params) #returns data as a data frame.

You could also use the helper function ```get_nass_yield()```:

    get_nass_yield(list("commodity_desc"="CORN", "agg_level_desc"="NATIONAL")) #gets US yields
...
Current numerical values are returned as strings, but we would like to return numeric values as floats. First we must make decisions about how to code the missing values
...
The FTP link also contains builds for: NASS' census (every 5 years, the next is 2017), or data for one of their specific sectors (CROPS, ECONOMICS, ANIMALS & PRODUCTS). At the time of this writing, files for the ENVIRONMENTAL and DEMOGRAPHICS sectors are not available.
